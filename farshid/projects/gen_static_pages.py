...
def page_files():
    """(rel path from the site root, absolute path), stable order."""
    for d in PAGE_DIRS:
...
def build_pages():
    ...
    for rel, src in page_files():
        ...
        pages.append({'slug': slug, 'route': route, 'permalink': _meta.get('permalink', ''),
                      ...
                      'url': f'{ORIGIN}/farshid/{rel}', 'md': f'/farshid/{rel}',
...
def write_permalinks(pages):
    ...
    rows = []
    for pg in pages:
        target = pg.get('route') or pg['slug']
