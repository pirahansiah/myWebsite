...
PERMALINK = {
    ...
    'research-tools': '/notes/slides/',
    'localAI': '/expert-coaching-resources/localAI/',          # the talks hub was /notes/slides/
}
...
def write_aliases(pairs, valid):
    ...
    lines = ['/* generated by gen_front_matter.py — legacy /notes/... URLs -> page routes. */',
             ...
             '  var A = {']
    items = sorted((k.lower(), v) for k, v in alias.items() if v)
...
def main():
    ...
    for path in pages:
        ...
        rel = os.path.relpath(path, ROOT).replace(os.sep, '/')      # content/wiki.md
        is_project = path.endswith('README.md')
        block, body, (title, permalink, description) = build_block(slug, path, vault, is_project)
        route = rel[:-3].replace('farshid/', '', 1)          # content/wiki, projects/rag/README
        route = route[len('content/'):] if route.startswith('content/') else route
        pairs.append((route, None if is_project else vault.find(slug, title)))
